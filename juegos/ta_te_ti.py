...
class Tateti:
    ...
    def evaluacion(self, estado: Estado) -> int:
        def cuenta_lineas(simbolo):
            ...
            lineas = [
                *estado,
                *[[estado[i][j] for i in range(3)] for j in range(3)],
                [estado[i][i] for i in range(3)],
                [estado[i][2 - i] for i in range(3)]
            ]
            for linea in lineas:
                if "X" in linea and "O" in linea:
                    continue  # línea bloqueada
                count = linea.count(simbolo)
                if count == 1:
                    x1 += 1
                elif count == 2:
                    x2 += 1
...
def minimax_ab(problema: Tateti, estado: Estado, profundidad_max: int = 2) -> Tuple[int, int]:
    ...
    if jugador == "X":
        sucs = {
            accion: minimax_max_ab(problema, problema.resultado(estado, accion), -math.inf, math.inf, 1, profundidad_max)
            for accion in problema.acciones(estado)
        }
        return max(sucs, key=sucs.get)
    else:
        sucs = {
            accion: minimax_min_ab(problema, problema.resultado(estado, accion), -math.inf, math.inf, 1, profundidad_max)
            for accion in problema.acciones(estado)
        }
...
def minimax_max_ab(problema, estado, alfa, beta, profundidad, profundidad_max):
    ...
    for accion in problema.acciones(estado):
        ...
        valor = max(valor, minimax_min_ab(problema, sucesor, alfa, beta, profundidad + 1, profundidad_max))
        if valor >= beta:
            return valor
        alfa = max(alfa, valor)
    return valor
...
def minimax_min_ab(problema, estado, alfa, beta, profundidad, profundidad_max):
    ...
    for accion in problema.acciones(estado):
        sucesor = problema.resultado(estado, accion)
        valor = min(valor, minimax_max_ab(problema, sucesor, alfa, beta, profundidad + 1, profundidad_max))
        if valor <= alfa:
            return valor
        beta = min(beta, valor)
    return valor
